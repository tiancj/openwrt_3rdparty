...
def fetch_ip_data():
    ...
    if (args.ip_file):
        ...
        # url='http://flora/delegated-apnic-latest' #debug
...
def decode_gfwlist(content):
    ...
    try:
        if b'.' in content:
            raise Exception()
        return base64.decodebytes(content).decode() # bytes -> str
    except:
        return content
...
def main():
    ...
    if args.user_rule:
        ...
        if not userrule_parts.scheme or not userrule_parts.netloc:
            ...
            user_rule = urlopen(args.user_rule, timeout=10).read().decode()

    if args.direct_rule:
        directrule_parts = urlsplit(args.direct_rule)
        if not directrule_parts.scheme or not directrule_parts.netloc:
            ...
            # Yeah, it's an URL, try to download it
            print('Downloading user rules file from %s' % args.user_rule)
            direct_rule = urlopen(args.direct_rule, timeout=10).read().decode()
        direct_rule = direct_rule.splitlines(False)
    else:
        direct_rule = []

    if args.localtld_rule:
        tldrule_parts = urlsplit(args.localtld_rule)
        if not tldrule_parts.scheme or not tldrule_parts.netloc:
            ...
            localtld_rule = urlopen(args.localtld_rule, timeout=10).read().decode()
